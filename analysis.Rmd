...
data.1 <- read_csv(file = "data/sightings.csv", col_types = col.def) %>%
  mutate(Month = month(Date),
         ...
         Year_Month_d = (Year_idx - 1) * 12 + Month) %>% 
  na.omit()
...
jags.data <- list(n = Counts,
                  ...
                  Year.Month = data.1$Year_Month_d,
                  max.Year.Season = max(data.1$Year_Season_d),
                  max.Year.Month = max(data.1$Year_Month_d),
                  alpha = rep(1, times = 10),
                  pi_N = rep(1, times = 200),
                  Nmin = 0, Nmax = 200)
...
for (k in 1:nrow(model.files)){
  ...
  if (!file.exists(out.file.name)){
    ...
    LOOIC.out.multi[[k]] <- compute.LOOIC(loglik.mat = jags.fit[[k]]$sims.list$loglik,
                                    MCMC.params = MCMC.params)
...
Pareto-k statistics are too high. The "best" model is also the worst fit model to the data. Something needs to be changed... 
